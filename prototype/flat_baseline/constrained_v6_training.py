"""Minimal train-only optimization and checkpoint workflow for V6."""
...
from .constrained_v6_checkpoint import (
    ...
    v6_model_metadata,
    validate_v6_checkpoint_field_set,
...
from .constrained_v6_training_config import (
    ConstrainedV6TrainingConfig,
    V6_TINY_SELECTION_IDENTITY,
)
...
V6_TINY_EVALUATION_MILESTONES = (100, 200)
V6_CHECKPOINT_FIELDS = V6_TINY_CHECKPOINT_FIELDS


class ConstrainedV6TrainingError(RuntimeError):
    """The bounded V6 workflow cannot continue safely."""
...
@dataclass(frozen=True)
class V6TrainingStepResult:
    metrics: dict
    gradient_parameter_names: tuple


@dataclass(frozen=True)
class V6LossSnapshot:
    metrics: dict


@dataclass(frozen=True)
class V6TinyOverfitResult:
    output_dir: str
    metrics_path: str
...
def load_v6_tiny_training_selection(
    corpus_dir,
    training_config: ConstrainedV6TrainingConfig,
    ...
    return select_v6_tiny_training_examples(
        physical,
...
def select_v6_tiny_training_examples(
    physical_examples,
    *,
    corpus_dir,
    split_name,
    ...
    if not ordered or any(item.partition != "train" for item in ordered):
        raise ConstrainedV6TrainingError(
    ...
    selection_identity = {
        ...
        "selection_rule": V6_TINY_SELECTION_IDENTITY,
...
def build_v6_optimizer(model, training_config, torch_module=torch):
    """Construct one AdamW group containing every V6 parameter exactly once."""
    ...
    if (
        ...
        raise ConstrainedV6TrainingError(
            ...
            "optimizer does not contain every intended V6 parameter",
        )
    return optimizer


def constrained_v6_training_step(
    model,
    optimizer,
    batch,
    model_config,
    ...
    *,
    ...
    examples_processed,
    torch_module=torch,
) -> V6TrainingStepResult:
    """Run one finite teacher-forced V6 optimization step."""
    ...
    return V6TrainingStepResult(metrics, gradient_names)


def constrained_v6_evaluation_snapshot(
    model,
    batch,
    model_config,
    training_config,
    ...
    *,
    ...
    try:
        ...
        return V6LossSnapshot(_loss_metrics(losses))
    finally:
        model.train(was_training)


def v6_checkpoint_payload(
    model,
    optimizer,
    model_config,
    training_config,
    ...
    *,
    ...
    """Build the strict version-6 checkpoint payload."""
    ...
    try:
        ...
        raise ConstrainedV6TrainingError(exc.code, exc.detail) from exc
    return payload


def save_v6_checkpoint(path, payload, torch_module=torch):
    checkpoint_kind = (
    ...
    try:
        validate_v6_checkpoint_field_set(payload, checkpoint_kind)
    except V6CheckpointValidationError as exc:
        raise ConstrainedV6TrainingError(exc.code, exc.detail) from exc
...
def run_v6_tiny_overfit(
    ...
    *,
    ...
    training_config = training_config or ConstrainedV6TrainingConfig()
    _validate_model_and_training_config(model_config, training_config)
    ...
    selection = load_v6_tiny_training_selection(
    ...
    optimizer = build_v6_optimizer(model, training_config, torch_module)
    batches = tuple(
    ...
    for step in range(1, training_config.maximum_steps + 1):
        ...
        if (
            step in V6_TINY_EVALUATION_MILESTONES
    ...
    save_v6_checkpoint(final_path, payload, torch_module)
...
def _validate_model_and_training_config(model_config, training_config):
    if not isinstance(model_config, ConstrainedProfileV6Config):
        raise TypeError("model_config must be ConstrainedProfileV6Config")
    if not isinstance(training_config, ConstrainedV6TrainingConfig):
        raise TypeError(
            "training_config must be ConstrainedV6TrainingConfig"
        )
...
def _step_metrics(
    ...
    if set(V6_TRAINING_LOSS_FIELDS) - set(metrics):
        raise AssertionError("V6 loss logging is incomplete")
    if not all(
...
def _aggregate_evaluation_snapshots(
    ...
    *,
    ...
    weighted = {name: 0.0 for name in V6_TRAINING_LOSS_FIELDS}
    family_count = 0
    parameter_count = 0
    example_count = 0
    for batch in batches:
        current_count = len(batch.family_ids)
        snapshot = constrained_v6_evaluation_snapshot(
            model,
            batch,
            model_config,
            training_config,
...
def _validate_v6_checkpoint(checkpoint, model_config, training_config, selection):
    try:
        validate_v6_checkpoint_payload(
            checkpoint,
            "tiny_overfit",
    ...
    for name, value in expected:
        if checkpoint[name] != value:
            raise ConstrainedV6TrainingError(
                "malformed_checkpoint",
                "{} differs from the requested V6 run".format(name),
            )
