"""Minimal train-only optimization and checkpoint workflow for V4."""
...
from .constrained_v4_training_config import (
    ConstrainedV4TrainingConfig,
...
V4_TINY_EVALUATION_MILESTONES = (100, 200)
V4_CHECKPOINT_FIELDS = {
    "checkpoint_version",
    "model_name",
...
class ConstrainedV4TrainingError(RuntimeError):
    """The bounded V4 workflow cannot continue safely."""
...
@dataclass(frozen=True)
class V4TrainingStepResult:
    metrics: dict
    gradient_parameter_names: tuple


@dataclass(frozen=True)
class V4LossSnapshot:
    metrics: dict


@dataclass(frozen=True)
class V4TinyOverfitResult:
    output_dir: str
    metrics_path: str
...
def load_v4_tiny_training_selection(
    corpus_dir,
    training_config: ConstrainedV4TrainingConfig,
    ...
    return select_v4_tiny_training_examples(
        physical,
...
def select_v4_tiny_training_examples(
    physical_examples,
    *,
    corpus_dir,
    split_name,
    ...
    if not ordered or any(item.partition != "train" for item in ordered):
        raise ConstrainedV4TrainingError(
    ...
    selection_identity = {
        ...
        "selection_rule": V4_TINY_SELECTION_IDENTITY,
...
def build_v4_optimizer(model, training_config, torch_module=torch):
    """Construct one AdamW group containing every V4 parameter exactly once."""
    ...
    if (
        ...
        raise ConstrainedV4TrainingError(
            ...
            "optimizer does not contain every intended V4 parameter",
        )
    return optimizer


def constrained_v4_training_step(
    model,
    optimizer,
    batch,
    model_config,
    ...
    *,
    ...
    examples_processed,
    torch_module=torch,
) -> V4TrainingStepResult:
    """Run one finite teacher-forced V4 optimization step."""
    ...
    losses = constrained_profile_v4_loss(
    ...
    return V4TrainingStepResult(metrics, gradient_names)


def constrained_v4_evaluation_snapshot(
    model,
    batch,
    model_config,
    training_config,
    ...
    *,
    ...
    try:
        ...
        return V4LossSnapshot(_loss_metrics(losses))
    finally:
...
def save_v4_checkpoint(path, payload, torch_module=torch):
...
def run_v4_tiny_overfit(
    ...
    *,
    ...
    model_config = model_config or ConstrainedProfileV4Config()
    training_config = training_config or ConstrainedV4TrainingConfig()
    _validate_model_and_training_config(model_config, training_config)
    ...
    selection = load_v4_tiny_training_selection(
    ...
    model = ConstrainedProfileV4Model(model_config).to(device)
    optimizer = build_v4_optimizer(model, training_config, torch_module)
    batches = tuple(
    ...
    for step in range(1, training_config.maximum_steps + 1):
        ...
        result = constrained_v4_training_step(
            model,
            optimizer,
            batch,
            model_config,
        ...
        if (
            step in V4_TINY_EVALUATION_MILESTONES
    ...
    save_v4_checkpoint(final_path, payload, torch_module)
    reloaded = ConstrainedProfileV4Model(model_config).to(device)
    reloaded_optimizer = build_v4_optimizer(
...
def _validate_model_and_training_config(model_config, training_config):
    if not isinstance(model_config, ConstrainedProfileV4Config):
        raise TypeError("model_config must be ConstrainedProfileV4Config")
    if not isinstance(training_config, ConstrainedV4TrainingConfig):
        raise TypeError(
            "training_config must be ConstrainedV4TrainingConfig"
        )
...
def _step_metrics(
    ...
    if set(V4_TRAINING_LOSS_FIELDS) - set(metrics):
        raise AssertionError("V4 loss logging is incomplete")
    if not all(
...
def _aggregate_evaluation_snapshots(
    ...
    *,
    ...
    weighted = {name: 0.0 for name in V4_TRAINING_LOSS_FIELDS}
    family_count = 0
    parameter_count = 0
    example_count = 0
    for batch in batches:
        current_count = len(batch.family_ids)
        snapshot = constrained_v4_evaluation_snapshot(
            model,
            batch,
            model_config,
            training_config,
        ...
        for name in V4_TRAINING_LOSS_FIELDS:
...
def _validate_v4_checkpoint(checkpoint, model_config, training_config, selection):
    if not isinstance(checkpoint, dict) or set(checkpoint) != V4_CHECKPOINT_FIELDS:
        raise ConstrainedV4TrainingError(
            "malformed_checkpoint", "V4 checkpoint fields are invalid"
        )
    if checkpoint["checkpoint_version"] != 4:
        raise ConstrainedV4TrainingError(
            "unsupported_checkpoint", "checkpoint_version must be 4"
        )
    if (
        checkpoint["epoch"] != 0
        ...
        raise ConstrainedV4TrainingError(
            "malformed_checkpoint", "V4 checkpoint state metadata is invalid"
        )
    expected = (
        ("model_name", model_config.model_name),
        ("model_config_version", model_config.model_config_version),
    ...
    for name, value in expected:
        if checkpoint[name] != value:
            raise ConstrainedV4TrainingError(
                ...
                "{} differs from the requested V4 run".format(name),
            )
