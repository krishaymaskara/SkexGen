"""Single strict checkpoint schema for all constrained V6 workflows."""
...
V6_TINY_CHECKPOINT_FIELDS = frozenset(
    V6_MODEL_METADATA_FIELDS + V6_TRAINING_STATE_FIELDS + V6_TINY_METADATA_FIELDS
)
V6_PILOT_CHECKPOINT_FIELDS = frozenset(
    V6_MODEL_METADATA_FIELDS + V6_TRAINING_STATE_FIELDS + V6_PILOT_METADATA_FIELDS
)


class V6CheckpointValidationError(ValueError):
    """A V6 payload violates the frozen checkpoint contract."""
...
def required_top_level_fields(checkpoint_kind):
    if checkpoint_kind == "tiny_overfit":
        return V6_TINY_CHECKPOINT_FIELDS
    if checkpoint_kind == "pilot_fixed_epoch":
        return V6_PILOT_CHECKPOINT_FIELDS
    raise V6CheckpointValidationError(
        "checkpoint kind {!r} is unsupported".format(checkpoint_kind)
    )


def validate_v6_checkpoint_field_set(checkpoint, checkpoint_kind):
...
def required_model_metadata_fields():
    return V6_MODEL_METADATA_FIELDS


def required_training_state_fields():
    return V6_TRAINING_STATE_FIELDS


def required_pilot_metadata_fields():
    return V6_PILOT_METADATA_FIELDS


def canonical_grammar_metadata():
    """Return deterministic, JSON-compatible V6 grammar provenance."""

    return node_grammar_metadata(V5_NODE_GRAMMAR)


def v6_model_metadata(model_config):
    """Construct the common model and grammar portion of every V6 payload."""
    ...
    return {
        ...
        "axis_geometry_contract_id": model_config.axis_geometry_contract_id,
        "axis_geometry_contract": axis_geometry_metadata(),
        "axis_construction_algorithm_id": (
            model_config.axis_construction_algorithm_id
        ),
        "axis_geometry_channel_indices": list(
            model_config.axis_geometry_channel_indices
        ),
        "axis_coordinate_space": model_config.axis_coordinate_space,
        "axis_raw_constrained_evidence_policy": (
            model_config.axis_raw_constrained_evidence_policy
        ),
...
def validate_v6_checkpoint_payload(
    ...
    *,
    ...
    """Validate common V6 schema, metadata, state types, and provenance."""

    validate_v6_checkpoint_field_set(checkpoint, checkpoint_kind)

    for name, expected in v6_model_metadata(model_config).items():
        if checkpoint[name] != expected:
            if name == "node_grammar_contract":
                _invalid(_grammar_mismatch(checkpoint[name], expected))
            _invalid("field {} differs from the frozen V6 contract".format(name))
    ...
    if checkpoint_kind == "tiny_overfit":
        for name in V6_TINY_METADATA_FIELDS:
            _validate_json_tree(checkpoint[name], name)
    return checkpoint


def validate_v6_pilot_checkpoint_state(
    checkpoint,
    pilot_config,
    train_example_count,
):
    ...
    for name in V6_PILOT_METADATA_FIELDS:
...
def _invalid(detail):
    raise V6CheckpointValidationError(detail)
