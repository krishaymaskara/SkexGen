"""Autonomous predicted-history decoding for the constrained V2 flat model."""
...
V2_ENCODED_MEMORY_SOURCE = "v2_encoder_quantized_memory"
V2_AUTONOMOUS_PREFIX_FEEDBACK = "v2_predicted_constrained_records"
_ENCODER_FIELDS = (
    "categorical_ids",
    "geometry",
    "geometry_mask",
...
class V2EncodedMemory:
    """Quantized V2 memory and immutable provenance needed for decoding."""
...
def greedy_decode_v2_from_memory(
    model: ConstrainedProfileV2Model,
    memory: V2EncodedMemory,
    *,
    node_counts: torch.Tensor,
    node_count_source: str,
) -> Tuple[V2AutonomousRawPrediction, ...]:
    """Greedily decode complete predicted V2 records to authorized lengths."""
...
def greedy_decode_v2(
    ...
    *,
    ...
    """Encode once and autonomously decode from the resulting V2 memory."""

    memory = encode_v2_to_memory(model, source_batch)
...
def validate_and_convert_v2_autonomous_prediction(
    prediction: V2AutonomousRawPrediction,
    *,
    max_operations: int,
) -> ConversionResult:
    """Run the existing strict converter through a V2 provenance adapter."""

    if not isinstance(prediction, V2AutonomousRawPrediction):
        raise TypeError(
            "prediction must be a V2AutonomousRawPrediction"
        )
    if prediction.prefix_feedback != V2_AUTONOMOUS_PREFIX_FEEDBACK:
        raise ValueError("V2 autonomous prediction has invalid feedback")
    if prediction.encoded_memory_source != V2_ENCODED_MEMORY_SOURCE:
        raise ValueError("V2 autonomous prediction has invalid memory source")
...
def _decode_one(
    ...
    return V2AutonomousRawPrediction(
        tuple(
...
def _validate_encoded_memory(model, encoded):
    if not isinstance(encoded, V2EncodedMemory):
        raise TypeError("memory must be V2EncodedMemory")
    ...
    if encoded.encoding_source != V2_ENCODED_MEMORY_SOURCE:
...
def _validate_prefix_output(model, output, output_length, memory):
    if not isinstance(output, ConstrainedProfileV2PrefixOutput):
        raise TypeError("decode_prefix returned an invalid V2 output")
    ...
    floats = (
        ...
        (
            ...
            expected + (NON_PROFILE_GEOMETRY_WIDTH,),
        ),
    )
