"""Versioned configuration for train-only constrained V5 smoke training."""
...
from .constrained_v5_config import (
...
V5_TINY_SELECTION_IDENTITY = "train_metadata_greedy_coverage_v1"
V5_TRAINING_PARTITION = "train"
V5_NO_VALIDATION_PARTITION = "none"
V5_TRAINING_DTYPES = ("float32",)
V5_TRAINING_DEVICES = ("auto", "cpu", "cuda")
V5_TINY_VQ_INITIALIZATIONS = ("normal",)


class ConstrainedV5TrainingConfigurationError(ValueError):
    """A V5 tiny-training setting violates the frozen contract."""


@dataclass(frozen=True)
class ConstrainedV5TrainingConfig:
    ...
    output_dir: str = "constrained_v5_tiny_overfit"
    ...
    training_partition: str = V5_TRAINING_PARTITION
    validation_partition: str = V5_NO_VALIDATION_PARTITION
    test_partition_accessed: bool = False
    split_name: str = "iid"
    tiny_subset_size: int = 8
    tiny_overfit_selection_identity: str = V5_TINY_SELECTION_IDENTITY
    ...
    def validate(self):
        fixed = (
            ...
            ("training_partition", V5_TRAINING_PARTITION),
            ("validation_partition", V5_NO_VALIDATION_PARTITION),
        ...
        for name, expected in fixed:
            if getattr(self, name) != expected:
                raise ConstrainedV5TrainingConfigurationError(
        ...
        if self.device not in V5_TRAINING_DEVICES:
            raise ConstrainedV5TrainingConfigurationError(
                "unsupported device"
            )
        if self.dtype not in V5_TRAINING_DTYPES:
            raise ConstrainedV5TrainingConfigurationError(
                "unsupported dtype"
            )
        if self.vq_initialization not in V5_TINY_VQ_INITIALIZATIONS:
            raise ConstrainedV5TrainingConfigurationError(
