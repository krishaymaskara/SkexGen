"""Integrated teacher-forced losses for the constrained-profile V2 model."""
...
class ConstrainedProfileV2Loss:
    """All retained V2 losses with profile terms reported separately."""
    ...
    def as_dict(self):
        return {
            ...
            "non_profile_geometry": self.non_profile_geometry,
...
def constrained_profile_v2_loss(
    output,
    target,
    profile_targets,
    config,
):
    """Return independently normalized V2 reconstruction losses."""
    ...
    if not isinstance(config, ConstrainedProfileV2Config):
        raise TypeError("config must be ConstrainedProfileV2Config")
    config.validate()
    validate_profile_targets_against_tensor_target(
    ...
    if len(output.categorical_logits) != len(
        ...
        raise ValueError("V2 must expose exactly five categorical heads")
    per_categorical_parts = tuple(
        ...
        for logits, target_index in zip(
    ...
    target_non_profile = select_non_profile_geometry(target["geometry"])
    authoritative_mask = (
        select_non_profile_geometry(target["geometry_mask"])
        & node_mask.unsqueeze(-1)
    )
    if not torch.equal(
        output.non_profile_geometry_mask, authoritative_mask
    ):
        raise ValueError(
            "non-profile geometry mask disagrees with authoritative target"
        )
    per_non_profile_geometry = _per_example_smooth_l1(
    ...
    per_total = (
        ...
        + config.geometry_loss_weight * per_non_profile_geometry
    ...
    per_example = {
        ...
        "non_profile_geometry": per_non_profile_geometry,
    ...
    for name, values in per_example.items():
        if not torch.isfinite(values).all():
            raise ValueError("{} V2 loss must be finite".format(name))
    return ConstrainedProfileV2Loss(
        per_total.mean(),
        per_node_type.mean(),
        per_categorical.mean(),
        per_non_profile_geometry.mean(),
        profile.family_loss,
