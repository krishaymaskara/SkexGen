"""Bounded V4 train-and-ordinary-IID-validation engineering pilot."""
...
from .constrained_v4_training import (
    V4_TRAINING_LOSS_FIELDS,
    build_v4_optimizer,
    constrained_v4_training_step,
    save_v4_checkpoint,
)
from .constrained_v4_training_config import ConstrainedV4TrainingConfig
from .constrained_v4_pilot_config import (
...
def pilot_optimization_config(config, configured_maximum_steps):
    config.validate()
    return ConstrainedV4TrainingConfig(
        seed=config.seed,
...
def teacher_forced_validation(
    ...
    loss_sums = {name: 0.0 for name in V4_TRAINING_LOSS_FIELDS}
    family_confusion = [
    ...
    try:
        ...
        with torch.no_grad():
            for start in range(0, len(partition.flat_examples), batch_size):
                ...
                predicted_nodes = output.node_type_logits.argmax(dim=-1)
                category_selection = select_node_conditioned_categorical_ids(
                    output.categorical_logits,
                    predicted_nodes,
                    target["node_mask"],
                )
                corrected_examples += int(
                    category_selection.correction_mask.any(dim=-1).any(dim=-1)
                    .sum().item()
                )
                _update_categorical_stats(
                    categorical_stats,
                    predicted_nodes.detach().cpu().tolist(),
                    category_selection.raw_categorical_argmax_ids.cpu().tolist(),
                    category_selection.node_conditioned_categorical_ids.cpu().tolist(),
...
def _decode_validation_examples(model, inputs, node_counts):
    ...
    for index in range(node_counts.numel()):
        ...
        try:
            results.append(greedy_decode_v4(
                model,
                source,
...
def run_constrained_v4_pilot(
    corpus_dir,
    pilot_config=None,
    model_config=None,
):
    config = pilot_config or ConstrainedV4PilotConfig()
    try:
        return _run_constrained_v4_pilot(
...
def _run_constrained_v4_pilot(
    corpus_dir,
    pilot_config=None,
    model_config=None,
):
    pilot_config = pilot_config or ConstrainedV4PilotConfig()
    ...
    optimizer = build_v4_optimizer(model, optimization)
    logger.write({
        "event": "run_metadata",
        "pilot_config": pilot_config.to_dict(),
        "model_config": model_config.to_dict(),
    ...
    for epoch in range(1, pilot_config.epochs + 1):
        ...
        save_v4_checkpoint(checkpoint, payload)
    ...
    reloaded_optimizer = build_v4_optimizer(reloaded, optimization)
    ...
    if final_path == selected_path:
        ...
        final_reloaded = ConstrainedProfileV4Model(model_config).to(device)
        final_optimizer = build_v4_optimizer(
