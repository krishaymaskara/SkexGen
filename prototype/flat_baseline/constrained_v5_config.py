"""Versioned configuration for the prefix-grammar constrained V5 model."""
...
CONSTRAINED_PROFILE_MODEL_CONFIG_VERSION = 5
CONSTRAINED_PROFILE_DECODER_CONTRACT_VERSION = 5
CONSTRAINED_PROFILE_CHECKPOINT_VERSION = 5
CONSTRAINED_PROFILE_FAMILY_ORDER = tuple(item.value for item in PROFILE_FAMILIES)
V5_LEARNED_GEOMETRY_CHANNEL_INDICES = V4_LEARNED_GEOMETRY_CHANNEL_INDICES
V5_BASE_GEOMETRY_CONTRACT_ID = V4_BASE_GEOMETRY_CONTRACT_ID


@dataclass(frozen=True)
class ConstrainedProfileV5Config(FlatBaselineConfig):
    ...
    learned_geometry_channel_indices: tuple = V5_LEARNED_GEOMETRY_CHANNEL_INDICES
    canonical_plane_contract_id: str = CANONICAL_PLANE_CONTRACT_ID
    base_geometry_contract_id: str = V5_BASE_GEOMETRY_CONTRACT_ID
    ...
    def validate(self):
        ...
        fixed = (
            ...
            ("learned_geometry_channel_indices", V5_LEARNED_GEOMETRY_CHANNEL_INDICES),
            ("canonical_plane_contract_id", CANONICAL_PLANE_CONTRACT_ID),
            ("base_geometry_contract_id", V5_BASE_GEOMETRY_CONTRACT_ID),
        ...
        for name, expected in fixed:
            if getattr(self, name) != expected:
                raise FlatBaselineConfigurationError(
                    "{} must equal {!r} for the V5 contract".format(name, expected)
                )
        if self.max_operations != 2:
            raise FlatBaselineConfigurationError(
                "V5 controlled node grammar requires max_operations == 2"
            )
