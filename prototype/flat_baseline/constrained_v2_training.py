"""Minimal train-only optimization and checkpoint workflow for V2."""
...
from .constrained_v2_training_config import (
...
class ConstrainedV2TrainingError(RuntimeError):
    """The bounded V2 workflow cannot continue safely."""
...
def load_v2_tiny_training_selection(
    corpus_dir,
    training_config: ConstrainedV2TrainingConfig,
    ...
    return select_v2_tiny_training_examples(
        physical,
...
def select_v2_tiny_training_examples(
    physical_examples,
    *,
    corpus_dir,
    split_name,
    ...
    selection_identity = {
        ...
        "selection_rule": V2_TINY_SELECTION_IDENTITY,
...
def build_v2_optimizer(model, training_config, torch_module=torch):
    """Construct one AdamW group containing every V2 parameter exactly once."""

    if not isinstance(model, ConstrainedProfileV2Model):
        raise TypeError("model must be ConstrainedProfileV2Model")
    training_config.validate()
    parameters = tuple(
    ...
    if (
        ...
        raise ConstrainedV2TrainingError(
            ...
            "optimizer does not contain every intended V2 parameter",
        )
    return optimizer


def constrained_v2_training_step(
    model,
    optimizer,
    batch,
    model_config,
    ...
    *,
    ...
    examples_processed,
    torch_module=torch,
) -> V2TrainingStepResult:
    """Run one finite teacher-forced V2 optimization step."""
...
def v2_checkpoint_payload(
    ...
    *,
    ...
    """Build the strict version-2 checkpoint payload."""
...
def save_v2_checkpoint(path, payload, torch_module=torch):
...
def run_v2_tiny_overfit(
    ...
    *,
    ...
    training_config = training_config or ConstrainedV2TrainingConfig()
    _validate_model_and_training_config(model_config, training_config)
    ...
    optimizer = build_v2_optimizer(model, training_config, torch_module)
    batches = tuple(
    ...
    save_v2_checkpoint(final_path, payload, torch_module)
    reloaded = ConstrainedProfileV2Model(model_config).to(device)
    reloaded_optimizer = build_v2_optimizer(
...
def _validate_model_and_training_config(model_config, training_config):
    if not isinstance(model_config, ConstrainedProfileV2Config):
        raise TypeError("model_config must be ConstrainedProfileV2Config")
    if not isinstance(training_config, ConstrainedV2TrainingConfig):
        raise TypeError(
            "training_config must be ConstrainedV2TrainingConfig"
        )
    ...
    expected = {
        ...
        "non_profile_geometry_loss_weight": model_config.geometry_loss_weight,
...
def _step_metrics(
    ...
    if set(V2_TRAINING_LOSS_FIELDS) - set(metrics):
        raise AssertionError("V2 loss logging is incomplete")
    if not all(
...
def _validate_v2_checkpoint(checkpoint, model_config, training_config, selection):
    if not isinstance(checkpoint, dict) or set(checkpoint) != V2_CHECKPOINT_FIELDS:
        raise ConstrainedV2TrainingError(
            "malformed_checkpoint", "V2 checkpoint fields are invalid"
        )
    if checkpoint["checkpoint_version"] != 2:
        raise ConstrainedV2TrainingError(
            "unsupported_checkpoint", "checkpoint_version must be 2"
        )
    if (
        checkpoint["epoch"] != 0
        ...
        raise ConstrainedV2TrainingError(
            "malformed_checkpoint", "V2 checkpoint state metadata is invalid"
        )
    expected = (
        ("model_name", model_config.model_name),
        ("model_config_version", model_config.model_config_version),
    ...
    for name, value in expected:
        if checkpoint[name] != value:
            raise ConstrainedV2TrainingError(
                ...
                "{} differs from the requested V2 run".format(name),
            )
