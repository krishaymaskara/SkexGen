"""V5 model identity with V4 trainable architecture and prefix grammar evidence."""
...
class ConstrainedProfileV5Output(ConstrainedProfileV4Output):
    """V4 numerical outputs plus the shifted prefix used for V5 reporting."""

    teacher_forced_prefix_node_ids: object


class ConstrainedProfileV5Model(ConstrainedProfileV4Model):
    """Parameter-shape-identical V4 successor with a V5 decoding identity."""

    def __init__(self, config=None):
        resolved = config or ConstrainedProfileV5Config()
        if not isinstance(resolved, ConstrainedProfileV5Config):
            raise TypeError("V5 model requires ConstrainedProfileV5Config")
        resolved.validate()
    ...
    def forward(
        ...
        prefix[:, 0] = 0
        return ConstrainedProfileV5Output(
