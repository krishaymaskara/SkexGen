"""Versioned configuration for train-only constrained V6 smoke training."""
...
from .constrained_v6_config import (
...
V6_TINY_SELECTION_IDENTITY = "train_metadata_greedy_coverage_v1"
V6_TRAINING_PARTITION = "train"
V6_NO_VALIDATION_PARTITION = "none"
V6_TRAINING_DTYPES = ("float32",)
V6_TRAINING_DEVICES = ("auto", "cpu", "cuda")
V6_TINY_VQ_INITIALIZATIONS = ("normal",)


class ConstrainedV6TrainingConfigurationError(ValueError):
    """A V6 tiny-training setting violates the frozen contract."""


@dataclass(frozen=True)
class ConstrainedV6TrainingConfig:
    ...
    output_dir: str = "constrained_v6_tiny_overfit"
    ...
    training_partition: str = V6_TRAINING_PARTITION
    validation_partition: str = V6_NO_VALIDATION_PARTITION
    test_partition_accessed: bool = False
    split_name: str = "iid"
    tiny_subset_size: int = 8
    tiny_overfit_selection_identity: str = V6_TINY_SELECTION_IDENTITY
    ...
    def validate(self):
        fixed = (
            ...
            ("training_partition", V6_TRAINING_PARTITION),
            ("validation_partition", V6_NO_VALIDATION_PARTITION),
        ...
        for name, expected in fixed:
            if getattr(self, name) != expected:
                raise ConstrainedV6TrainingConfigurationError(
        ...
        if self.device not in V6_TRAINING_DEVICES:
            raise ConstrainedV6TrainingConfigurationError(
                "unsupported device"
            )
        if self.dtype not in V6_TRAINING_DTYPES:
            raise ConstrainedV6TrainingConfigurationError(
                "unsupported dtype"
            )
        if self.vq_initialization not in V6_TINY_VQ_INITIALIZATIONS:
            raise ConstrainedV6TrainingConfigurationError(
