"""Versioned configuration for the constrained-profile V3 flat model."""
...
CONSTRAINED_PROFILE_MODEL_NAME = "B0-FLAT-CONSTRAINED-PROFILE-CANONICAL-PLANE-v3"
CONSTRAINED_PROFILE_MODEL_CONFIG_VERSION = 3
CONSTRAINED_PROFILE_DECODER_CONTRACT_VERSION = 3
CONSTRAINED_PROFILE_CHECKPOINT_VERSION = 3
CONSTRAINED_PROFILE_FAMILY_ORDER = tuple(
    family.value for family in PROFILE_FAMILIES
)
V3_LEARNED_GEOMETRY_CHANNEL_INDICES = (33, 34, 35, 36, 37, 38)


@dataclass(frozen=True)
class ConstrainedProfileV3Config(FlatBaselineConfig):
    """Frozen V3 identity layered over the shared flat-model dimensions."""
    ...
    learned_geometry_channel_indices: tuple = V3_LEARNED_GEOMETRY_CHANNEL_INDICES
        ...
        for name, expected in expected_metadata:
            if getattr(self, name) != expected:
                raise FlatBaselineConfigurationError(
                    "{} must equal {!r} for the V3 contract".format(
                        name, expected
                    )
                )
        _finite_nonnegative(
