"""Minimal train-only optimization and checkpoint workflow for V3."""
...
from .constrained_v3 import ConstrainedProfileV3Model
from .constrained_v3_config import ConstrainedProfileV3Config
from .constrained_v3_losses import constrained_profile_v3_loss
from .constrained_v3_training_config import (
    ConstrainedV3TrainingConfig,
    V3_TINY_SELECTION_IDENTITY,
)
...
V3_TINY_EVALUATION_MILESTONES = (100, 200)
V3_CHECKPOINT_FIELDS = {
    "checkpoint_version",
    "model_name",
...
class ConstrainedV3TrainingError(RuntimeError):
    """The bounded V3 workflow cannot continue safely."""
...
@dataclass(frozen=True)
class V3TrainingStepResult:
    metrics: dict
    gradient_parameter_names: tuple


@dataclass(frozen=True)
class V3LossSnapshot:
    metrics: dict


@dataclass(frozen=True)
class V3TinyOverfitResult:
    output_dir: str
    metrics_path: str
...
def load_v3_tiny_training_selection(
    corpus_dir,
    training_config: ConstrainedV3TrainingConfig,
    ...
    return select_v3_tiny_training_examples(
        physical,
...
def select_v3_tiny_training_examples(
    physical_examples,
    *,
    corpus_dir,
    split_name,
    ...
    if not ordered or any(item.partition != "train" for item in ordered):
        raise ConstrainedV3TrainingError(
    ...
    selection_identity = {
        ...
        "selection_rule": V3_TINY_SELECTION_IDENTITY,
...
def build_v3_optimizer(model, training_config, torch_module=torch):
    """Construct one AdamW group containing every V3 parameter exactly once."""

    if not isinstance(model, ConstrainedProfileV3Model):
        raise TypeError("model must be ConstrainedProfileV3Model")
    training_config.validate()
    parameters = tuple(
    ...
    if (
        ...
        raise ConstrainedV3TrainingError(
            ...
            "optimizer does not contain every intended V3 parameter",
        )
    return optimizer


def constrained_v3_training_step(
    model,
    optimizer,
    batch,
    model_config,
    ...
    *,
    ...
    examples_processed,
    torch_module=torch,
) -> V3TrainingStepResult:
    """Run one finite teacher-forced V3 optimization step."""
    ...
    losses = constrained_profile_v3_loss(
    ...
    return V3TrainingStepResult(metrics, gradient_names)


def constrained_v3_evaluation_snapshot(
    model,
    batch,
    model_config,
    training_config,
    ...
    *,
    ...
    try:
        ...
        return V3LossSnapshot(_loss_metrics(losses))
    finally:
        model.train(was_training)


def v3_checkpoint_payload(
    model,
    optimizer,
    model_config,
    training_config,
    ...
    *,
    ...
    """Build the strict version-3 checkpoint payload."""
...
def save_v3_checkpoint(path, payload, torch_module=torch):
...
def run_v3_tiny_overfit(
    ...
    *,
    ...
    model_config = model_config or ConstrainedProfileV3Config()
    training_config = training_config or ConstrainedV3TrainingConfig()
    _validate_model_and_training_config(model_config, training_config)
    ...
    selection = load_v3_tiny_training_selection(
    ...
    model = ConstrainedProfileV3Model(model_config).to(device)
    optimizer = build_v3_optimizer(model, training_config, torch_module)
    batches = tuple(
    ...
    for step in range(1, training_config.maximum_steps + 1):
        ...
        result = constrained_v3_training_step(
            model,
            optimizer,
            batch,
            model_config,
        ...
        if (
            step in V3_TINY_EVALUATION_MILESTONES
    ...
    save_v3_checkpoint(final_path, payload, torch_module)
    reloaded = ConstrainedProfileV3Model(model_config).to(device)
    reloaded_optimizer = build_v3_optimizer(
...
def _validate_model_and_training_config(model_config, training_config):
    if not isinstance(model_config, ConstrainedProfileV3Config):
        raise TypeError("model_config must be ConstrainedProfileV3Config")
    if not isinstance(training_config, ConstrainedV3TrainingConfig):
        raise TypeError(
            "training_config must be ConstrainedV3TrainingConfig"
        )
...
def _step_metrics(
    ...
    if set(V3_TRAINING_LOSS_FIELDS) - set(metrics):
        raise AssertionError("V3 loss logging is incomplete")
    if not all(
...
def _aggregate_evaluation_snapshots(
    ...
    *,
    ...
    weighted = {name: 0.0 for name in V3_TRAINING_LOSS_FIELDS}
    family_count = 0
    parameter_count = 0
    example_count = 0
    for batch in batches:
        current_count = len(batch.family_ids)
        snapshot = constrained_v3_evaluation_snapshot(
            model,
            batch,
            model_config,
            training_config,
        ...
        for name in V3_TRAINING_LOSS_FIELDS:
...
def _validate_v3_checkpoint(checkpoint, model_config, training_config, selection):
    if not isinstance(checkpoint, dict) or set(checkpoint) != V3_CHECKPOINT_FIELDS:
        raise ConstrainedV3TrainingError(
            "malformed_checkpoint", "V3 checkpoint fields are invalid"
        )
    if checkpoint["checkpoint_version"] != 3:
        raise ConstrainedV3TrainingError(
            "unsupported_checkpoint", "checkpoint_version must be 3"
        )
    if (
        checkpoint["epoch"] != 0
        ...
        raise ConstrainedV3TrainingError(
            "malformed_checkpoint", "V3 checkpoint state metadata is invalid"
        )
    expected = (
        ("model_name", model_config.model_name),
        ("model_config_version", model_config.model_config_version),
    ...
    for name, value in expected:
        if checkpoint[name] != value:
            raise ConstrainedV3TrainingError(
                ...
                "{} differs from the requested V3 run".format(name),
            )
