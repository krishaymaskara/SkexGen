"""Leakage-safe teacher-forced prediction conversion for the V3 flat model."""
...
class V3PredictedProfileTensors:
...
@dataclass(frozen=True)
class V3PredictedNodeTensors:
    """Complete canonical node records built only from V3 predictions."""
...
@dataclass(frozen=True)
class V3TeacherForcedRawPrediction(RawDecodedPrediction):
    """Existing raw record plus V3-specific profile prediction evidence."""
...
def canonicalize_v3_predicted_profiles(
    family_ids: torch.Tensor,
    raw_parameters: torch.Tensor,
    sketch_mask: torch.Tensor,
) -> V3PredictedProfileTensors:
    ...
    return V3PredictedProfileTensors(
        family_ids,
        constrained,
...
def construct_v3_predicted_node_tensors(
    node_type_ids: torch.Tensor,
    ...
    profiles = canonicalize_v3_predicted_profiles(
...
def v3_teacher_forced_predictions(
    output: ConstrainedProfileV3Output,
    *,
    node_mask: torch.Tensor,
    node_count_source: str,
) -> tuple:
    """Convert aligned V3 outputs into raw teacher-forced predictions.
...
def validate_and_convert_v3_teacher_forced_prediction(
    prediction: V3TeacherForcedRawPrediction,
    *,
    max_operations: int,
) -> ConversionResult:
    """Run the existing strict converter through a provenance-only view."""

    if not isinstance(prediction, V3TeacherForcedRawPrediction):
        raise TypeError(
            "prediction must be a V3TeacherForcedRawPrediction"
        )
    if prediction.prefix_feedback != TEACHER_FORCED_PREFIX_FEEDBACK:
        raise ValueError(
            "V3 teacher-forced prediction has invalid prefix provenance"
        )
    compatible = replace(
...
def _prediction_row(
    ...
    return V3TeacherForcedRawPrediction(
        tuple(
            int(value)
...
def _validate_predicted_node_inputs(
    ...
    if retained_categorical_ids.device != node_type_ids.device:
        raise ValueError("predicted node tensors must share device")
    # A plane-category ID is checked by the canonical constructor so an
    # applicable malformed prediction receives the V3 structured error.
...
def _validate_conversion_inputs(output, node_mask, node_count_source):
    if not isinstance(output, ConstrainedProfileV3Output):
        raise TypeError("output must be ConstrainedProfileV3Output")
    ...
    if (
        ...
        raise ValueError("V3 must expose five categorical logits")
