"""Target-free predicted-history decoding for prefix-grammar V6."""
...
def encode_v6_to_memory(model, source_batch):
...
def greedy_decode_v6_from_memory(
    model,
    memory,
    *,
    node_counts,
    node_count_source,
) -> Tuple[V6AutonomousRawPrediction, ...]:
    _validate_v6_model(model)
...
def greedy_decode_v6(
    ...
    *,
    ...
    return greedy_decode_v6_from_memory(
        model,
        memory,
...
def validate_and_convert_v6_autonomous_prediction(
    ...
    if not isinstance(prediction, V6AutonomousRawPrediction):
        raise NodeGrammarError(
            "invalid_v6_node_selection", "prediction has the wrong V6 type"
        )
    if prediction.prefix_feedback != V6_AUTONOMOUS_PREFIX_FEEDBACK:
        raise NodeGrammarError(
...
def validate_v6_autonomous_evaluation_result(
    ...
    if prediction.node_count != requested_node_count:
        raise NodeGrammarError(
            "invalid_v6_node_selection", "generated count differs from request"
        )
    validate_v6_prediction_contract(
        prediction, require_complete_sequence=True
    )
