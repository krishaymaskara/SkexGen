"""Teacher-forced V4 flat model with constrained controlled profiles."""
...
REMAINING_GEOMETRY_INDICES = V4_LEARNED_GEOMETRY_CHANNEL_INDICES
...
if REMAINING_GEOMETRY_WIDTH != 6 or REMAINING_GEOMETRY_INDICES != tuple(range(33, 39)):
    raise AssertionError("the V4 learned geometry contract must be channels 33-38")


@dataclass(frozen=True)
class ConstrainedProfileV4Output:
    """Teacher-forced V4 predictions and training-only canonical profiles."""
...
@dataclass(frozen=True)
class ConstrainedProfileV4PrefixOutput:
    """V4 predictions for BOS plus a generated-record prefix."""
...
def scatter_remaining_geometry(values: torch.Tensor) -> torch.Tensor:
    """Scatter learned V4 channels 33-38 into serialized width 39."""
...
def select_remaining_geometry(values: torch.Tensor) -> torch.Tensor:
    """Gather serialized channels 33-38 in the exact learned V4 order."""
...
class ConstrainedProfileV4Model(FlatMixedVQModel):
    """V4 successor retaining V1 encoder/VQ/causal-decoder internals."""

    def __init__(self, config=None):
        resolved = config or ConstrainedProfileV4Config()
        if not isinstance(resolved, ConstrainedProfileV4Config):
            raise TypeError("V4 model requires ConstrainedProfileV4Config")
        ...
        # remain in the V4 state dictionary.
    ...
    def forward(
        ...
        return ConstrainedProfileV4Output(
            decoded_states,
    ...
    def decode_prefix(
        ...
        """Decode BOS plus complete preceding V4 node records."""

        batch_size, prefix_length = _validate_v4_prefix_inputs(
            self,
            memory,
            categorical_prefix,
            geometry_prefix,
        ...
        profile_output = self.profile_heads(decoded_states)
        return ConstrainedProfileV4PrefixOutput(
            decoded_states,
            self.node_type_head(decoded_states),
            tuple(
                head(decoded_states)
...
def _validate_v4_prefix_inputs(
    model,
    memory,
    categorical_prefix,
    geometry_prefix,
