"""Versioned configuration for train-only constrained V3 smoke training."""
...
V3_TINY_SELECTION_IDENTITY = "train_metadata_greedy_coverage_v1"
V3_TRAINING_PARTITION = "train"
V3_NO_VALIDATION_PARTITION = "none"
V3_TRAINING_DTYPES = ("float32",)
V3_TRAINING_DEVICES = ("auto", "cpu", "cuda")
V3_TINY_VQ_INITIALIZATIONS = ("normal",)


class ConstrainedV3TrainingConfigurationError(ValueError):
    """A V3 tiny-training setting violates the frozen contract."""


@dataclass(frozen=True)
class ConstrainedV3TrainingConfig:
    ...
    output_dir: str = "constrained_v3_tiny_overfit"
    ...
    training_partition: str = V3_TRAINING_PARTITION
    validation_partition: str = V3_NO_VALIDATION_PARTITION
    test_partition_accessed: bool = False
    split_name: str = "iid"
    tiny_subset_size: int = 8
    tiny_overfit_selection_identity: str = V3_TINY_SELECTION_IDENTITY
    ...
    def validate(self):
        fixed = (
            ...
            ("training_partition", V3_TRAINING_PARTITION),
            ("validation_partition", V3_NO_VALIDATION_PARTITION),
        ...
        for name, expected in fixed:
            if getattr(self, name) != expected:
                raise ConstrainedV3TrainingConfigurationError(
        ...
        if self.device not in V3_TRAINING_DEVICES:
            raise ConstrainedV3TrainingConfigurationError(
                "unsupported device"
            )
        if self.dtype not in V3_TRAINING_DTYPES:
            raise ConstrainedV3TrainingConfigurationError(
                "unsupported dtype"
            )
        if self.vq_initialization not in V3_TINY_VQ_INITIALIZATIONS:
            raise ConstrainedV3TrainingConfigurationError(
