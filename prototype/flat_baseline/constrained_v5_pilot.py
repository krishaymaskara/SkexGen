"""Bounded V5 train-and-ordinary-IID-validation engineering pilot."""
...
from .constrained_v5_training import (
    V5_TRAINING_LOSS_FIELDS,
    build_v5_optimizer,
    constrained_v5_training_step,
    save_v5_checkpoint,
)
from .constrained_v5_training_config import ConstrainedV5TrainingConfig
from .constrained_v5_pilot_config import (
...
def pilot_optimization_config(config, configured_maximum_steps):
    config.validate()
    return ConstrainedV5TrainingConfig(
        seed=config.seed,
...
def teacher_forced_validation(
    ...
    loss_sums = {name: 0.0 for name in V5_TRAINING_LOSS_FIELDS}
    family_confusion = [
    ...
    try:
        ...
        with torch.no_grad():
            for start in range(0, len(partition.flat_examples), batch_size):
                ...
                grammar_predictions = v5_teacher_forced_predictions(
                    output,
    ...
    try:
        ...
        for start in range(0, len(partition.flat_examples), batch_size):
            ...
            for local_index, (example, raw) in enumerate(
                ...
                result = validate_and_convert_v5_autonomous_prediction(
                ...
                if (
                    ...
                    raise V5AutonomousEvaluationError(
...
def _decode_validation_examples(model, inputs, node_counts):
    ...
    for index in range(node_counts.numel()):
        ...
        try:
            results.append(greedy_decode_v5(
                model,
                source,
...
def _finish_node_grammar_stats(stats):
    ...
    if result["constrained_grammar_violation_count"] != 0:
        raise ConstrainedV5PilotError(
            ...
            "V5 selected an illegal node type",
        )
    return result
...
def run_constrained_v5_pilot(
    ...
    config = pilot_config or ConstrainedV5PilotConfig()
    try:
        return _run_constrained_v5_pilot(
...
def _run_constrained_v5_pilot(
    corpus_dir,
    pilot_config=None,
    model_config=None,
):
    pilot_config = pilot_config or ConstrainedV5PilotConfig()
    ...
    for epoch in range(1, pilot_config.epochs + 1):
        ...
        save_v5_checkpoint(checkpoint, payload)
    ...
    reloaded_optimizer = build_v5_optimizer(reloaded, optimization)
    ...
    if final_path == selected_path:
        ...
        final_reloaded = ConstrainedProfileV5Model(model_config).to(device)
        final_optimizer = build_v5_optimizer(
