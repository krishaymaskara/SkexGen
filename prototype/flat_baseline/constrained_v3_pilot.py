"""Bounded V3 train-and-ordinary-IID-validation engineering pilot."""
...
from .constrained_v3_autonomous import (
    greedy_decode_v3,
    validate_and_convert_v3_autonomous_prediction,
)
...
from .constrained_v3_training import (
    V3_TRAINING_LOSS_FIELDS,
    build_v3_optimizer,
    constrained_v3_training_step,
    save_v3_checkpoint,
)
from .constrained_v3_training_config import ConstrainedV3TrainingConfig
from .constrained_v3_pilot_config import (
...
def pilot_optimization_config(config, configured_maximum_steps):
    config.validate()
    return ConstrainedV3TrainingConfig(
        seed=config.seed,
...
def teacher_forced_validation(
    ...
    loss_sums = {name: 0.0 for name in V3_TRAINING_LOSS_FIELDS}
    family_confusion = [
        ...
        for start in range(0, len(partition.flat_examples), batch_size):
            ...
            for example, raw in zip(physical, raw_predictions):
                if isinstance(raw, ReferencePlaneGeometryError):
                    ...
                    continue
                result = validate_and_convert_v3_autonomous_prediction(
...
def _decode_validation_examples(model, inputs, node_counts):
    ...
    for index in range(node_counts.numel()):
        ...
        try:
            results.append(greedy_decode_v3(
                model,
                source,
...
def run_constrained_v3_pilot(
    corpus_dir,
    pilot_config=None,
    model_config=None,
):
    config = pilot_config or ConstrainedV3PilotConfig()
    try:
        return _run_constrained_v3_pilot(
...
def _run_constrained_v3_pilot(
    corpus_dir,
    pilot_config=None,
    model_config=None,
):
    pilot_config = pilot_config or ConstrainedV3PilotConfig()
    ...
    optimizer = build_v3_optimizer(model, optimization)
    logger.write({
        "event": "run_metadata",
        "pilot_config": pilot_config.to_dict(),
        "model_config": model_config.to_dict(),
    ...
    for epoch in range(1, pilot_config.epochs + 1):
        ...
        save_v3_checkpoint(checkpoint, payload)
    ...
    reloaded_optimizer = build_v3_optimizer(reloaded, optimization)
    ...
    if final_path == selected_path:
        ...
        final_reloaded = ConstrainedProfileV3Model(model_config).to(device)
        final_optimizer = build_v3_optimizer(
