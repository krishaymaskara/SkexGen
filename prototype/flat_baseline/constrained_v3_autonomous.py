"""Autonomous predicted-history decoding for the constrained V3 flat model."""
...
from .constrained_v3 import (
    REMAINING_GEOMETRY_WIDTH,
    ConstrainedProfileV3Model,
    ConstrainedProfileV3PrefixOutput,
)
from .constrained_v3_conversion import (
    V3TeacherForcedRawPrediction,
    construct_v3_predicted_node_tensors,
)
from .conversion import ConversionResult, validate_and_convert_raw_prediction


V3_ENCODED_MEMORY_SOURCE = "v3_encoder_quantized_memory"
V3_AUTONOMOUS_PREFIX_FEEDBACK = "v3_predicted_constrained_records"
_ENCODER_FIELDS = (
    "categorical_ids",
    "geometry",
    "geometry_mask",
...
class V3EncodedMemory:
    """Quantized V3 memory and immutable provenance needed for decoding."""
...
@dataclass(frozen=True)
class V3AutonomousRawPrediction(V3TeacherForcedRawPrediction):
    """Autonomous raw evidence with explicit encoded-memory provenance."""

    encoded_memory_source: str


def encode_v3_to_memory(
    model: ConstrainedProfileV3Model,
    ...
    result = V3EncodedMemory(
        memory,
        code_indices,
        V3_ENCODED_MEMORY_SOURCE,
...
def greedy_decode_v3_from_memory(
    model: ConstrainedProfileV3Model,
    memory: V3EncodedMemory,
    *,
    node_counts: torch.Tensor,
    node_count_source: str,
) -> Tuple[V3AutonomousRawPrediction, ...]:
    """Greedily decode complete predicted V3 records to authorized lengths."""
...
def greedy_decode_v3(
    ...
    *,
    ...
    """Encode once and autonomously decode from the resulting V3 memory."""

    memory = encode_v3_to_memory(model, source_batch)
    return greedy_decode_v3_from_memory(
        model,
        memory,
...
def validate_and_convert_v3_autonomous_prediction(
    prediction: V3AutonomousRawPrediction,
    *,
    max_operations: int,
) -> ConversionResult:
    """Run the existing strict converter through a V3 provenance adapter."""

    if not isinstance(prediction, V3AutonomousRawPrediction):
        raise TypeError(
            "prediction must be a V3AutonomousRawPrediction"
        )
    if prediction.prefix_feedback != V3_AUTONOMOUS_PREFIX_FEEDBACK:
        raise ValueError("V3 autonomous prediction has invalid feedback")
    if prediction.encoded_memory_source != V3_ENCODED_MEMORY_SOURCE:
        raise ValueError("V3 autonomous prediction has invalid memory source")
...
def _decode_one(
    ...
    for position in range(node_count):
        ...
        records = construct_v3_predicted_node_tensors(
            node_type_ids,
            retained_ids,
    ...
    return V3AutonomousRawPrediction(
        tuple(
...
def _validate_encoded_memory(model, encoded):
    if not isinstance(encoded, V3EncodedMemory):
        raise TypeError("memory must be V3EncodedMemory")
    ...
    if encoded.encoding_source != V3_ENCODED_MEMORY_SOURCE:
...
def _validate_prefix_output(model, output, output_length, memory):
    if not isinstance(output, ConstrainedProfileV3PrefixOutput):
        raise TypeError("decode_prefix returned an invalid V3 output")
