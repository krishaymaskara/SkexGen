"""Versioned configuration for train-only constrained V2 smoke training."""
...
from .constrained_v2_config import (
...
V2_TINY_SELECTION_IDENTITY = "train_metadata_greedy_coverage_v1"
V2_TRAINING_PARTITION = "train"
V2_NO_VALIDATION_PARTITION = "none"
V2_TRAINING_DTYPES = ("float32",)
V2_TRAINING_DEVICES = ("auto", "cpu", "cuda")
V2_TINY_VQ_INITIALIZATIONS = ("normal",)


class ConstrainedV2TrainingConfigurationError(ValueError):
    """A V2 tiny-training setting violates the frozen contract."""


@dataclass(frozen=True)
class ConstrainedV2TrainingConfig:
    ...
    non_profile_geometry_loss_weight: float = 1.0
    ...
    training_partition: str = V2_TRAINING_PARTITION
    validation_partition: str = V2_NO_VALIDATION_PARTITION
    test_partition_accessed: bool = False
    split_name: str = "iid"
    tiny_subset_size: int = 8
    tiny_overfit_selection_identity: str = V2_TINY_SELECTION_IDENTITY
    ...
    def validate(self):
        fixed = (
            ...
            ("training_partition", V2_TRAINING_PARTITION),
            ("validation_partition", V2_NO_VALIDATION_PARTITION),
        ...
        for name, expected in fixed:
            if getattr(self, name) != expected:
                raise ConstrainedV2TrainingConfigurationError(
        ...
        if self.device not in V2_TRAINING_DEVICES:
            raise ConstrainedV2TrainingConfigurationError(
                "unsupported device"
            )
        if self.dtype not in V2_TRAINING_DTYPES:
            raise ConstrainedV2TrainingConfigurationError(
                "unsupported dtype"
            )
        if self.vq_initialization not in V2_TINY_VQ_INITIALIZATIONS:
            raise ConstrainedV2TrainingConfigurationError(
