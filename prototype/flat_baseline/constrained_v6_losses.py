"""Numerically V4-equivalent losses for the V6 model identity."""
...
def constrained_profile_v6_loss(output, target, profile_targets, config):
    if not isinstance(output, ConstrainedProfileV6Output):
        raise TypeError("output must be ConstrainedProfileV6Output")
    if not isinstance(config, ConstrainedProfileV6Config):
        raise TypeError("config must be ConstrainedProfileV6Config")
    config.validate()
