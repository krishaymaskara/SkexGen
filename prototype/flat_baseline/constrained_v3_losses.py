"""Integrated teacher-forced losses for the constrained-profile V3 model."""
...
class ConstrainedProfileV3Loss:
    """All retained V3 losses with profile terms reported separately."""
...
def constrained_profile_v3_loss(
    output,
    target,
    profile_targets,
    config,
):
    """Return independently normalized V3 reconstruction losses."""
    ...
    if not isinstance(config, ConstrainedProfileV3Config):
        raise TypeError("config must be ConstrainedProfileV3Config")
    config.validate()
    validate_profile_targets_against_tensor_target(
    ...
    if len(output.categorical_logits) != len(
        ...
        raise ValueError("V3 must expose exactly five categorical heads")
    ...
    for name, values in per_example.items():
        if not torch.isfinite(values).all():
            raise ValueError("{} V3 loss must be finite".format(name))
    return ConstrainedProfileV3Loss(
        per_total.mean(),
        per_node_type.mean(),
