...
| Current reviewed revision | Repaired-sufficiency job `3345280` remains an immutable valid train-only scientific failure; ADR-0011 job `3346513` and representation-screen job `3350017` remain incomplete; closed-form job `3351524` completed validly without establishing prospective accessibility; accepted ADR-0013 records technically valid engineering job `3351837` with a prospective-authorization deviation and authorizes exactly one separately submitted exact-commit operation-template-train-only grid-magnitude sufficiency execution; no outcome auto-authorizes another job, repair, Stage 6, C8, or protected access |
...
| Status recorded | August 17, 2026 |
...
| GE1 Stage 6 structure-only comparison | Accepted prospective [ADR-0014](decisions/ADR-0014-ge1-stage6-structure-only-comparison.md) authorizes source-only implementation/preparation of `GE1-STAGE6-STRUCTURE-ONLY-COMPARISON-v1`. The additive implementation includes V5 grammar scoring, exact-count full credit, batch-local memory intervention, directional interpretation, a governed two-package narrow builder and receipt, independently audited narrow loaders, hardware-bound CPU/CUDA timing-v2 with three-seed priority, separate CPU/GPU producer paths, strict epoch-200 recovery and checkpoint bundle, producer/finalizer verification, tests, audits, and non-submitting runners. No real narrow package or timing was created. Exact reviewed hashes and Adroit roots, GPU allocation/hardware review, transfer, scientific training/inference, development access, submission, RR, Stage 7, ER, and every protected partition remain unresolved or unauthorized |
