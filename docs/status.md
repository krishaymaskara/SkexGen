...
| GE1 Stage 6 structure-only comparison | Accepted prospective [ADR-0014](decisions/ADR-0014-ge1-stage6-structure-only-comparison.md) authorizes implementation/preparation of `GE1-STAGE6-STRUCTURE-ONLY-COMPARISON-v1`. The additive implementation includes V5 grammar scoring, exact-count full credit, batch-local memory intervention, directional interpretation, separately audited narrow indexes/loaders, strict epoch-200 recovery and checkpoint bundle, job-scoped incomplete evidence, producer/finalizer verification, tests, audits, and non-submitting runners. Exact reviewed indexes/hashes and narrow Adroit roots, prospective timing, final allocation, exact future commit/runner hashes, transfer, scientific training/inference, development access, submission, RR, Stage 7, ER, and every protected partition remain unresolved or unauthorized |
