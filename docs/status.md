...
| Current reviewed revision | [ADR-0017](decisions/ADR-0017-ge1-exploratory-stage6-autonomous-stop-execution.md) reclassifies failed memory-to-count probe job `3355776` as informative while preserving its typed-graph seed-2028 `0.751/0.726` result. Exact-commit implementation validation and timing plus a mandatory two-epoch full-lifecycle smoke are authorized. The designated reviewer explicitly withheld the epoch-200 producer submission pending review of those results; preparing its command is allowed, running it is not. Any later interpretation remains forced inconclusive; RR, ER, IID, Stage 7, C8, CAD execution, retry, and confirmatory claims remain unauthorized |
...
| GE1 exploratory autonomous-stop Stage 6 | Accepted [ADR-0017](decisions/ADR-0017-ge1-exploratory-stage6-autonomous-stop-execution.md) adds opt-in continuation after recorded train-gate failure, immutable exploratory/smoke artifact markers, exact-commit timing, and a mandatory two-epoch lifecycle smoke. The epoch-200 ordinal-magnitude/autonomous-stop producer is held for a separate reviewer decision after those results are reported and must not be submitted in the current implementation task. Any later comparison number is exploratory and the finalizer must force inconclusive; no confirmatory result or protected-partition access is authorized |
