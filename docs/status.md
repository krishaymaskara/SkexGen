...
| Current reviewed revision | ADR-0014 CPU producer job `3354961` stopped at its train-reliability gate before development and produced no final comparison; train-gate postmortem job `3355134` preserved the authenticated prior evidence; zero-memory job `3355342` at exact commit `2eaabf2e5a88fc62f559c914123694539a93e188` completed with a valid five-file artifact and the documented local-evidence qualification. Exact-zero train output remained structurally informative, but the diagnostic creates no threshold, Stage 6 result, repair, retry, development, C8, or protected-access authority |
...
| GE1 Stage 6 structure-only comparison | Accepted prospective [ADR-0014](decisions/ADR-0014-ge1-stage6-structure-only-comparison.md) froze `GE1-STAGE6-STRUCTURE-ONLY-COMPARISON-v1`. Later reviewer acts authorized narrow packages, timing-v2, and CPU producer job `3354961`; that producer trained six arm/seed conditions but stopped at `stage6_train_reliability_failure` before development or finalization. Postmortem job `3355134` preserved the train evidence. [Zero-memory job `3355342`](experiments/ge1_stage6_zero_memory_3355342.md) completed read-only at exact commit `2eaabf2e5a88fc62f559c914123694539a93e188`: pooled train `P_zero` was `0.8378378378378378` flat and `0.5855855855855856` typed graph versus authenticated `P_true=1.0` for both and `P_mean=0.7903357903357904` / `0.8361998361998362`. This supports only a train-side decoder/scalar-path candidate and memory-contribution statement. No finalized comparison, new gate, repair, retry, development access, RR, Stage 7, C8, ER, or protected access is authorized |
