...
| GE1 grid-anchored magnitude repair | Accepted [ADR-0013](decisions/ADR-0013-ge1-grid-anchored-ordinal-operation-magnitude-repair.md) and its [implementation/execution contract](specifications/ge1_grid_magnitude_repair.md) freeze ordinal class-only v1, no residual, equal `1.0` extrusion/revolve weights, train-only tiny-before-scaled evaluation, and grid-only exclusion of compact magnitude channels 4/5 while preserving axis channels. Corpus-free job `3351837` at exact commit `bdb7148dc4ebe751bda1a66cd167fcf035495762` is technically valid but carries a recorded prospective-authorization deviation. Exactly one separately submitted clean detached exact-commit `GE1-C7-GRID-MAGNITUDE-SUFFICIENCY-v1` run is authorized after runner validation and transfer; no scientific job has yet been submitted under this authorization, and no outcome auto-authorizes another job, repair, Stage 6, C8, or protected access |
