...
| GE1 closed-form readout execution | Adroit job `3351524` is recorded by the designated reviewer as the completed readout, reported as **not establishing controlled magnitude-class accessibility from the tested decoder-state features**. This entry is reviewer-supplied: the job artifact, input hashes, per-feature outcomes, and permutation summaries are **not yet cited in this repository** and must be attached before any scientific execution relies on them |
| GE1 grid-anchored magnitude repair | Proposed [ADR-0013](decisions/ADR-0013-ge1-grid-anchored-ordinal-operation-magnitude-repair.md) and its [implementation contract](specifications/ge1_grid_magnitude_repair.md) add `GE1-OPERATION-MAGNITUDE-GRID-ORDINAL-v1`, protocol `GE1-C7-GRID-MAGNITUDE-SUFFICIENCY-v1`, and schema `GE1-CHECKPOINT-v2`. The corpus-free implementation now connects the ordinal term to the real teacher-forced training loss, preserves axis supervision while routing magnitude applicability to versioned ordinal metrics, adds structured diagnostics and a separate synthetic integration suite, and shares one structural source audit with a strengthened unsubmitted one-hour CPU runner covering all five regressions. Local PyTorch cases remain deferred to the authoritative container; no engineering job, scientific execution, accessibility claim, repair result, Stage 6, or C8 is authorized |
