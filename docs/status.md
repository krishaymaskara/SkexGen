...
| Current reviewed revision | [ADR-0017](decisions/ADR-0017-ge1-exploratory-stage6-autonomous-stop-execution.md) reclassifies failed memory-to-count probe job `3355776` as informative while preserving its typed-graph seed-2028 `0.751/0.726` result. It authorizes exact-commit implementation validation and timing, then a mandatory two-epoch full-lifecycle smoke before one schema-marked epoch-200 exploratory autonomous-stop producer and finalizer. Any resulting interpretation is forced inconclusive; RR, ER, IID, Stage 7, C8, CAD execution, retry, and confirmatory claims remain unauthorized |
...
| GE1 exploratory autonomous-stop Stage 6 | Accepted [ADR-0017](decisions/ADR-0017-ge1-exploratory-stage6-autonomous-stop-execution.md) adds opt-in continuation after recorded train-gate failure, immutable exploratory/smoke artifact markers, exact-commit timing, a mandatory two-epoch lifecycle smoke, and one epoch-200 ordinal-magnitude/autonomous-stop development execution. The comparison number is exploratory and the finalizer must force inconclusive; no confirmatory result or protected-partition access is authorized |
