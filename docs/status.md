...
| Current reviewed revision | [ADR-0017](decisions/ADR-0017-ge1-exploratory-stage6-autonomous-stop-execution.md) is implemented at exact commit `42d5c489c7236031ff18dfd13ca88af428ca7510`. CPU validation job `3355816` passed; timing job `3355819` retained CPU and seeds 2026–2028; two-epoch producer/finalizer jobs `3355822`/`3355823` finalized cleanly as an inconclusive, result-ineligible smoke. The designated reviewer explicitly withheld the epoch-200 producer submission; its command is prepared but was not run. RR, ER, IID, Stage 7, C8, CAD execution, retry, and confirmatory claims remain unauthorized |
...
| GE1 closed-form representation readout | Accepted [ADR-0012](decisions/ADR-0012-ge1-closed-form-representation-readout.md) freezes `GE1-C7-CLOSED-FORM-READOUT-v2`: its common A/B baseline uses one finite-precision weak order, tie collapse, strict-inversion rejection, and ordered fold anchors rather than numeric coordinate midpoints; separate A/B analyses remain descriptive; 24 pipelines, 8 structured nonexecuted records, exactly 16 primary synchronized-permutation hypotheses, and metric-specific centered maxT remain frozen; ADR-0011 and the earlier screen remain incomplete |
| GE1 closed-form readout execution | Adroit job `3351524` at exact commit `4dff7e0ed1fea7076b64e2513689088aff4d8078` completed `GE1-C7-CLOSED-FORM-READOUT-v2` and did **not establish controlled magnitude-class accessibility from the tested features** under any arm/type prospective rule. The supplied transcript and finalized JSON are cited in the [execution record](experiments/ge1_closed_form_readout_3351524.md); the other four artifact files were not supplied locally, so independent manifest-to-JSON checksum linkage remains unavailable. The job does not complete ADR-0011/the screen or authorize repair, Stage 6, C8, or protected access |
| GE1 grid-anchored magnitude repair | Accepted [ADR-0013](decisions/ADR-0013-ge1-grid-anchored-ordinal-operation-magnitude-repair.md) freezes ordinal class-only v1. Scientific job `3352404` failed revolve fidelity and consumed the scientific authorization. Corpus/checkpoint-free trajectory job `3353008` validly found that classes 2–4 never decoded above class 1 within 200 generated-state updates. The reviewer authorizes preparation, without transfer or submission in this turn, of a separate [2,000-update horizon diagnostic](specifications/ge1_grid_ordinal_horizon_diagnostic.md) for classes 2–4. No production change, scientific retry, further diagnostic, repair, protected access, Stage 6, or C8 is authorized |
| GE1 grid softmax magnitude classification | Accepted [ADR-0015](decisions/ADR-0015-ge1-grid-softmax-operation-magnitude-classification.md) adds opt-in `GE1-OPERATION-MAGNITUDE-GRID-SOFTMAX-v1`: two independent `Linear(model_dim, 5)` heads, argmax decode, and cross-entropy replacing the shared-scalar CORAL readout whose four global thresholds made classes 2–4 jointly unreachable in job `3353008`. The ADR-0013 ordinal identity, its `GE1-CHECKPOINT-v2` schema, its sufficiency protocol, and every recorded result are unchanged and still runnable; the new identity is checkpoint incompatible under `GE1-CHECKPOINT-v3`. Implementation, corpus-free synthetic validation, and documentation only — no scientific run, runner, submission, Stage 6, C8, repair, or protected access is authorized |
| GE1 autonomous stop and unconstrained node decoding | Accepted [ADR-0016](decisions/ADR-0016-ge1-autonomous-stop-and-unconstrained-node-decoding.md) adds opt-in `GE1-PAD-TERMINATED-UNCONSTRAINED-NODES-v1`, supervises one `<pad>` terminator, removes target node counts and grammar transition masks from that identity, advances decoder/output/checkpoint contracts to V4/v3/v4, and prospectively removes the non-identifying cross-arm train-ceiling predicate without replacement. ADR-0017 additively expands only the explicitly marked exploratory execution boundary; all other ADR-0016 restrictions remain |
| GE1 autonomous-stop engineering validation | [Adroit CPU job `3355790`](experiments/ge1_autonomous_stop_validation_3355790.md) passed at exact commit `288327e42b8575f774b9df668df288d3fa71ae33`: 19/19 focused and 734/734 complete graph-encoder tests, exact expected skips, every sibling suite, and all repository gates. Job `3355776` remains a failed diagnostic, but ADR-0017 no longer treats it as a blocking prerequisite for the one exploratory sequence |
| GE1 exploratory autonomous-stop Stage 6 | Accepted [ADR-0017](decisions/ADR-0017-ge1-exploratory-stage6-autonomous-stop-execution.md) adds opt-in continuation after recorded train-gate failure and immutable exploratory/smoke artifact markers. Exact-commit CPU validation [job `3355816`](experiments/ge1_exploratory_stage6_validation_3355816.md) passed. Re-timing [job `3355819`](experiments/ge1_exploratory_stage6_timing_3355819.md) retained CPU and seeds 2026–2028. The two-epoch producer/finalizer smoke [jobs `3355822`/`3355823`](experiments/ge1_exploratory_stage6_smoke_3355822_3355823.md) finalized cleanly with `stage6_result_eligible=false` and `interpretation_category=inconclusive`. The epoch-200 command is prepared but was not submitted and remains held for a separate reviewer decision; no confirmatory result or protected-partition access is authorized |
| GE1 Stage 6 structure-only comparison | Accepted prospective [ADR-0014](decisions/ADR-0014-ge1-stage6-structure-only-comparison.md) froze `GE1-STAGE6-STRUCTURE-ONLY-COMPARISON-v1`. [Producer job `3354961`](experiments/ge1_stage6_structure_only_producer_3354961.md) trained six arm/seed conditions but stopped before development or finalization. [Postmortem job `3355134`](experiments/ge1_stage6_train_gate_postmortem_3355134.md) authenticated pooled train values: flat `P_true/P_shuffle/P_mean=1.0/0.6887796887796888/0.7903357903357904` passed; typed graph `1.0/0.7387387387387387/0.8361998361998362` failed only mean-memory against the inclusive `0.80` gate. [Zero-memory job `3355342`](experiments/ge1_stage6_zero_memory_3355342.md) then found pooled `P_zero=0.8378378378378378` flat and `0.5855855855855856` typed graph. ADR-0017 now authorizes only the marked exploratory sequence described above; no confirmatory comparison, repair, retry, RR, Stage 7, C8, ER, or other protected access is authorized |
