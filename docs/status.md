...
| GE1 grid-anchored magnitude repair | Accepted [ADR-0013](decisions/ADR-0013-ge1-grid-anchored-ordinal-operation-magnitude-repair.md) freezes ordinal class-only v1. Scientific job `3352404` at commit `de80d93208a1a98196a38d2af5cd47afd525d14f` validly finalized with both tiny exact gates passing, both revolve-fidelity gates failing on 90-degree targets decoded as 45 degrees, and scaled access closed; it consumed the one scientific authorization. Separate corpus/checkpoint-free trajectory job `3352969` passed all tests but failed its preservation gate before diagnostic invocation because its governed README was absent from the allowlist. The reviewer authorizes exactly one corrected replacement execution. No scientific retry, further engineering retry, production change, repair, protected access, Stage 6, or C8 is authorized |
